...
# Each e.student / e.course access below triggers a separate lazy-load
# query the first time it's touched -> classic N+1 (fixed further down with joinedload).
...
# joinedload tells SQLAlchemy to pull student & course into the SAME query
# via SQL JOINs, so accessing e.student / e.course below hits no extra DB calls.
