...
# FastAPI dependency: any route that adds `Depends(get_current_user)` becomes
# a protected endpoint — it forces a valid Bearer token and resolves it to
# the actual User row before the route body runs.
...
async def lifespan(app:FastAPI):
    # Runs once on startup: creates all tables from the SQLAlchemy models
    # if they don't already exist. Everything after `yield` would run on shutdown.
...
async def login(form_data:OAuth2PasswordRequestForm = Depends(),db:AsyncSession = Depends(get_db)):
    ...
    # Credentials check passed — issue a signed JWT identifying this user.
