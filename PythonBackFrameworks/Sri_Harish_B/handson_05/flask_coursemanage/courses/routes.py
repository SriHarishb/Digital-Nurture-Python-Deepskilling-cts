...
# Blueprint groups all /api/courses/* routes so they can be registered on the
# Flask app in one line (see app.py) instead of defining routes directly on `app`.
courses_bp = Blueprint('courses', __name__, url_prefix='/api/courses')


# Shared helper so every success response has the same {status, data} shape.
